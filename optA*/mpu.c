...
void mld_s(unsigned int md, void *base_address, unsigned int row_stride) {
    ...
    for (int x = 0; x < mlmul*nlmul; x++) {
        ...
        base_address = base_address_origin + (mlmul_idx * row_stride * lambda + nlmul_idx*K) * elem_width;
...
void fmmac_s(unsigned int md, unsigned int ms1, unsigned int ms2) {
    ...
    for (int x = 0; x < mlmul*nlmul; x++) {
        ...
        //print_reg_as_float(ms1+mlmul_idx+mlmul);
        ...
        if (lambda!=K) {
            float *A = (float *) reg(ms1+mlmul_idx+mlmul);

            for (int i = 0; i < lambda; i++) {
                for (int j = 0; j < K; j++) {   
                    for (int k = 0; k < K; k++) {
                        C[i * step + j] += A[i * step + k] * B[(k+K) * step + j];
                    }
                }
            } 
        }
